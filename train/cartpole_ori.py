...
def trainBarrier(latent_sde, Barrier,  batch_size=64,  device='cuda', Test=False):
    ...
    for it in range(_BARRIER_ITERATIONS):
        ...
        #     ini_state = copy.copy(INI_STATE)
        #     ini_state[0] += np.random.uniform(-0.1, 0.1, 1)
        #     ini_state[1] += np.random.uniform(-0.05, 0.05, 1)
        ...
        for _ in range(batch_size):
            ini_state = copy.copy(INI_STATE)
            ini_state[0] += np.random.uniform(-0.1, 0.1)
            ini_state[1] += np.random.uniform(-0.05, 0.05)
            _x0.append(ini_state)

        _x0 = torch.tensor(_x0, dtype=torch.float32, device=device)
...
def main(
        ...
        noise_std=0.01,
        adjoint=False,
        train_dir='D:/UoS/yixuan_icml/safe_unknown_control/train/cartpole/',
        method="euler",
):
    ...
    checkpoint = torch.load('D:/UoS/yixuan_icml/safe_unknown_control/train/cartpole/model_6200.pth')
